...
def find_corners_of_largest_polygon(img):
	...
	bottom_right, br = max(enumerate([pt[0][0] + pt[0][1] for pt in polygon]), key=operator.itemgetter(1))
	top_left, _tl = min(enumerate([pt[0][0] + pt[0][1] for pt in polygon]), key=operator.itemgetter(1))
	bottom_left, bl = min(enumerate([pt[0][0] - pt[0][1] for pt in polygon]), key=operator.itemgetter(1))
	top_right, tr = max(enumerate([pt[0][0] - pt[0][1] for pt in polygon]), key=operator.itemgetter(1))
...
def get_digits(img, squares, size,numbers):
	...
	for square in squares:
		num =extract_digit(img, square, size).astype(np.float32)
		if count>=8:
			x=(count//9)
			y=(count%9)
			if (num.mean()!=0):
...
def parse_grid(path, numbers):
	original = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
	processed = pre_process_image(original,True)
	corners = find_corners_of_largest_polygon(processed)
	cropped = crop_and_warp(original, corners)
	squares = infer_grid(cropped)
	digits = get_digits(cropped, squares, 125,numbers)
	print(str(digits))
   #  show_digits(digits)
...
if __name__ == '__main__':
	...
	test_chars, test_classes, test_segments = ocr.ocr(test_image, show_steps=False)
	# print(test_chars[0])
